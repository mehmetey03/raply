...
def main():
    ...
    for sezon_id, sezon_adi in super_lig_sezonlar.items():
        for hafta in super_lig_haftalar.get(sezon_id, range(1, 39)):
    ...
    # TFF 1. Lig
    all_urls.extend(get_birinci_lig_urls_dynamically())

    grouped_results = {}
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        future_results = executor.map(fetch_and_parse, all_urls)
    ...
    for group_title, lines in sorted(grouped_results.items()):
        safe_name = group_title.replace('/', '-').replace(' ', '_')
        folder_path = os.path.join(output_folder, safe_name)
        os.makedirs(folder_path, exist_ok=True)
        file_path = os.path.join(folder_path, f"{safe_name}.m3u")
    ...
    # Master M3U
    master_path = os.path.join(output_folder, 'all_leagues.m3u')
    ...
    print(f"Tüm ligler kaydedildi: {len(grouped_results)} dosya oluşturuldu")
